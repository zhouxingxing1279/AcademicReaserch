...
def learned_prediction(z,u,config,model,certificate):
    accel_bound=validate_envelope(certificate,model,config)
    ...
    nn_remainder=.5*model.directional_second_bound(s-ds,s+ds,ds)
    ...
    return out,{'model_radius':w,'remainder_radius':remainder,'neural_remainder_radius':h*nn_remainder,
                'linear_radius':np.abs(linear).sum(axis=1)}


class LearnedZonotopeSMF(ZonotopeSMF):
    def __init__(self,prior,observation,config,model,certificate,max_generators=60):
        validate_envelope(certificate,model,config)
        self.model,self.certificate=model,certificate
    ...
    def advance(self,actual_u,observation):
        ...
        prediction,diag=learned_prediction(self.zonotope,actual_u,self.config,self.model,self.certificate)
