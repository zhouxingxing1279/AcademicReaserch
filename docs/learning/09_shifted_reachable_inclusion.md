...
这里的缺测恒等式只描述位置通道的更新。本项目角度/角速度每步可测，完整后验仍应与这些测量条带求交；第6节缺测数值案例省略这些收紧，是保守包含探针，不是完整传感器更新实现。
...
后续进展见 [10 固定模板 CZ](10_fixed_template_cz.md)：固定维度等价编码和滚动外包链已给出；试验模板的几何精度仍未通过，先处理有限传播方向的充分性。
