# 34 执行器权限与扰动合同审计：静态反馈失败不等于系统物理不可行

日期：2026-09-24。承接第 33 章。

## 1. 本轮核心问题

第 33 章已经严格否定一个具体 hover-DARE ancillary feedback：在当前全域 residual contract 下，固定高推力路径的有限 disturbance reachable support 已经使 |Ke|>0.08。

但这只否定一个反馈结构。如果继续写成“执行器权限不足，因此不存在可行 Tube MPC”，会把 controller-synthesis failure 错写成 plant-level impossibility。
