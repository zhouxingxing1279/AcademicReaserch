...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('training',type=Path);parser.add_argument('--output',type=Path,required=True);parser.add_argument('--propagation',choices=('affine','legacy'),default='affine');args=parser.parse_args()
    filter_class=LearnedZonotopeSMF
    if args.propagation=='legacy':
        from smf_mpc.learned_filter import LearnedZonotopeSMF as filter_class
    ...
    for seed in (70001,70002,70003):
        ...
        for scenario in ('S0','S1','S2'):
            ...
            f=filter_class(prior,plant.observe(truth[0],0,idx,potential[0],config),config,model,cert,60)
