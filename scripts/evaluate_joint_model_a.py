#!/usr/bin/env python3
"""Affine-bound model-A replay; retained baseline logs use exactly the same sources."""
import argparse,csv,gzip,hashlib,io,json,sys,time
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.path.insert(0,str(ROOT/'src'))
import numpy as np
from smf_mpc import plant
from smf_mpc.affine_model import AffineResidualMLP as ResidualMLP
from smf_mpc.affine_filter import AffineLearnedZonotopeSMF as LearnedZonotopeSMF
...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('training',type=Path);parser.add_argument('--output',type=Path,required=True);parser.add_argument('--propagation',choices=('global','local'),default='local');args=parser.parse_args()
    ...
    table=None
    if args.propagation=='local':
        from smf_mpc.local_envelope import LocalEnvelope
        from smf_mpc.local_filter import LocalLearnedZonotopeSMF
        arrays=np.load(args.training/'envelope_leaves.npz')
        table=LocalEnvelope(cert,model,config,arrays['lower'],arrays['upper'],arrays['acceleration_halfwidth'])
    ...
    for seed in (70001,70002,70003):
        ...
        for scenario in ('S0','S1','S2'):
            ...
            initial=plant.observe(truth[0],0,idx,potential[0],config)
            f=LearnedZonotopeSMF(prior,initial,config,model,cert,60) if table is None else LocalLearnedZonotopeSMF(prior,initial,config,model,cert,table,60)
    ...
    save_json(args.output/'summary.json',{'propagation':args.propagation,'scope':'fixed model A affine-bound ablation, no real-time guarantee','model_sha256':model.digest(),'runs':summaries})
