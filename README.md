...
按 [12 控制权限与输入结构](docs/learning/12_feedback_authority_and_input_structure.md) 继续：已证明旧独立全域仿射盒在任意反馈下都无法从初始中心无限期保持原域，最迟209步失效。已构造保留推力—姿态关系、更紧的一步外包，并验证已知变输入下72方向运输的滚动包含。下一步实现保留反馈律与观测分支的联合乘积外包，再检查状态/输入收紧、策略移位和终端合同；不再为旧盒搜索覆盖初始中心的终端证书，控制接口闭合前不进入RL训练。
