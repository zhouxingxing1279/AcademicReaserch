...
**主方案已确定（2026-09-15）：[强化学习调度集合验证的输出反馈 MPC](docs/learning/03_selected_plan.md)。学习位置固定为离散验证任务调度，首版不学习动力学；小型精确序列验证后采用 Double DQN。下一步只交付控制接口与有限调度环境规范；历史候选比较及下一步指令以主方案为准，尚未证明学习优势或完整安全。**

## 当前研究：间歇位置测量下的神经动力学建模与集员输出反馈 MPC

**当前入口：[研究总框架](docs/research_framework.md)。先固定研究主问题、模块接口和证明依赖，再推进具体算法。**

主问题：在相同物理知识、测量历史和计算预算下，学习动力学模型能否减少集员估计的外包冗余，并在既定鲁棒 MPC 框架中产生可验证收益？

用户随后授权的 [方向选择机制核查](docs/learning/01_direction_selection_probe.md) 已完成：八个精确几何实例显示相关方向可减少冗余，但单目标准确支持时，直接查询约束法向的非学习规则已最优。尚未训练网络或证明学习收益。下一步只检查实际多目标预算瓶颈与强基线差距；需要的控制接口仍由 F1 提供。
...
按 [研究总框架 F1](docs/research_framework.md)，核查至多两个已有控制基础的假设与当前模型、估计、缺测和输入合同是否相容，选定一个主路线，交付假设映射和剩余证明义务。暂不继续 09 的限幅不变集求解。历史 P2/P5 和学习优势仍未通过，框架调整不等于这些问题已经解决。
